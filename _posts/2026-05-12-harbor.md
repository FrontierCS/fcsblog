...
![FrontierCS on Harbor trace summary]({{ 'assets/img/2026-05-12-harbor/leaderboard-summary.svg' | relative_url }})
...
Kimi is slightly ahead in final score on this set of runs. The traces show a sharper difference. Claude takes more steps on average, uses 251K output tokens per task on average, and hits the token/output cap in 31 zero-score runs. Kimi's output token count is not available in this batch of traces, but its tool-call trajectory suggests a more tool-driven style: 70.6 tool calls per task on average, compared with 42.2 for Claude.
